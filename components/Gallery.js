...
export default function Gallery({ works }) {
  ...
  useEffect(() => {
    ...
    const updateThread = () => {
      ...
      const drawHead = window.scrollY + window.innerHeight * 0.88;
      const travel = Math.max(rect.height * 0.80, 1);
      const progress = Math.min(1, Math.max(0, (drawHead - galleryTop) / travel));
              ...
              <button type="button" className="featured-art" onClick={() => setActive(work)} aria-label={`Open ${work.title}`}>
                <span className="featured-image-stage">
                  <Image
                    ...
                    alt={work.alt}
                    fill
                    sizes="(max-width: 700px) 76vw, (max-width: 1000px) 64vw, 54vw"
