...
export default function Gallery({ works }) {
  ...
  useEffect(() => {
    ...
    const updateThread = () => {
      ...
      const galleryHeight = Math.max(rect.height, 1);

      // Drive the gold line from absolute page scroll rather than the viewport-relative
      // rectangle alone. The thread deliberately runs ahead of the eye and reaches the end
      // before the final works leave the viewport, so even a fast wheel/trackpad scroll feels
      // immediate instead of as if the light is chasing the page.
      const start = galleryTop - window.innerHeight * 0.90;
      const finish = galleryTop + galleryHeight - window.innerHeight * 1.18;
      const raw = Math.min(1, Math.max(0, (window.scrollY - start) / Math.max(finish - start, 1)));
      const accelerated = Math.min(1, Math.pow(raw, 0.58) * 1.12 + (raw > 0 ? 0.025 : 0));

      gallery.style.setProperty('--thread-progress', accelerated.toFixed(4));
    };
